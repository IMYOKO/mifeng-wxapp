    ...
    <view class='wrap'>
      <view>
        <view class='date-show'>
          <image class = "tap-icon" src='../static/images/ic_home_calendar_previous_page.png' mode='aspectFit'  @tap='lastMonth' />
          <view class  ="dete-now">{{year}}年  -   {{month}}月</view>
          <image class = "tap-icon" src='../static/images/ic_home_calendar_next_page.png' mode='aspectFit' @tap='nextMonth' />
        </view>
      </view>
      <view class='header'>
        <view v-for="(item, index) in date" :key ="index" :class="(index == todayIndex) && isTodayWeek ? 'weekMark' : ''">{{item}}<view></view></view>
      </view>
      <view class='date-box'>
...
export default {
  ...
  async onLoad(options) {
    ...
    this.year = year;
    this.month = month;
    let this_month_day = '' + year + '-' + ( (month) < 10 ? ( '0' + (month) ) : (month) ) + '-' + '01';
    await this.getUnuseDate(this_month_day,free, advertise_machine_id);
    this.isToday =  '' + year + '-' + ( (month) < 10 ? ( '0' + (month) ) : (month) ) + '-' + now.getDate();
		this.dateInit(); 
  },
  methods: {
    //选择日期
    ...
    async lastMonth(){
      ...
      this.month = (month + 1);
      this.$apply();
      let this_month_day = '' + this.year + '-' + ( (this.month) < 10 ? ( '0' + (this.month) ) : (this.month) ) + '-' + '01';
    ...
    dateInit(setYear,setMonth){
      ...
      for(let i = 0; i < arrLen; i++){
        if(i >= startWeek){
          num = i - startWeek + 1;
          obj = {
            isToday: '' + year + '-' + ( (month + 1) < 10 ? ( '0' + (month + 1) ) : (month + 1) ) + '-' + ( num < 10 ? ( '0' + num ) : num),
            dateLine:'' + year + '-' + ( (month + 1) < 10 ? ( '0' + (month + 1) ) : (month + 1) ) + '-' + ( num < 10 ? ( '0' + num ) : num),
            dateNum: num,
            weight: 5
          }
        }else{
      ...
      for(var i in tempDateArr){
        if(tempDateArr[i].dateLine){
          ...
          if(util.getTimeStamp(tempDateArr[i].dateLine) < util.getTimeStamp(this.isToday)){
            //时间未大于今天  不能选取
