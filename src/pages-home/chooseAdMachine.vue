<template>
  <view class="chooseAdMachine">
    <view class="search-view" @tap="clickSite">
      ...
      <image class = "search-icon" src="../static/images/ic_home_search_1.png" @tap.stop="clickSearch" />
    </view>
    <view class = "banner" >
      <scroll-view class = "sv" scroll-x="true">
        <view class="item" :class="advertise_machine_label_type_id==''?'selected':''" @tap="clickSx">全部</view>
        <view class="item" :class="advertise_machine_label_type_id==item.id?'selected':''" v-for="(item, index) in machineType" :key="index" @tap="clickSx(item.id)">{{item.name}}</view>
      </scroll-view>
    ...
    <view class = "pop-classify" v-if="showClassify">
      ...
      <view class = "classify-ct">
        <view class = "classify-item" :class="advertise_machine_label_type_id==''?'selected':''" @tap="clickSx">全部</view>
        <view class = "classify-item" :class="advertise_machine_label_type_id==item.id?'selected':''" :key="index" v-for="(item, index) in machineType" @tap="clickSx(item.id)">{{item.name}}</view>
      </view>
    </view>
    <view class="modal-mask" @tap="clickHidden" catchtouchmove="preventTouchMove" v-if="showClassify"></view>
  </view>
</template>
