<template>
  <view>
    ...
    <view
      ...
      :key="index"
      @click="clickDetail(item.orderId)"
    >
      <view class="top">
        <view
          class="type"
        >广告类型：{{item.materialType === 1 ? '竖屏图片' : item.materialType === 2 ? '横屏图片' : item.materialType === 3 ? '竖屏视频' : item.materialType === 4 ? '横屏视频' : '组合素材'}}</view>
        <view class="dot"></view>
        <view
          class="status"
        >{{item.status === 0 ? '待支付' : item.status === 1 ? '待投放' : item.status === 2 ? '投放中' : item.status === 3 ? '已完成' : '已关闭'}}</view>
      </view>
      <view class="middle">
        <image
          class="img"
    ...
    <view class="pop" v-if="showPop2">
      ...
      <view class="ct" :class="payMoney>money?'grey':''" @tap="clickPayWay(2)">
        <image class="ct-icon" src="../static/images/ic_home_payment_wallet.png" />
        <view class="ct-text">余额支付（余额：¥{{money}}）</view>
      </view>
      <view class="ct" :class="payMoney>integral?'grey':''" @tap="clickPayWay(3)">
...
export default {
  ...
  methods: {
    ...
    clickPayWay(pay_type) {
      ...
      if (pay_type === 3) {
        //积分支付
        if (this.payMoney > this.integral) {
          tip.toast("可支付积分不足");
          return;
        }
      }
